...
def main():
    ...
    while True:
        ...
        # ---------- HOST / LOBBY ----------
        if choice == "host":
            result = lobby_menu_loop(port=5000)   # choose your port
            if result == "start":
                # Placeholder gameplay loop; replace with your real game
                run_placeholder_gameplay(screen, clock)
                # After gameplay, we return to the main menu
                continue
            elif result == "back":
                # Back to main menu
                continue
        ...
        # ---------- JOIN ----------
            ...
            if isinstance(result, tuple) and result and result[0] == "connect":
                ...
                print(f"[JOIN] Attempt connect to {ip}:{port}")
